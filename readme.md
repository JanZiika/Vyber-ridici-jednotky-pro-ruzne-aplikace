...
| :----------------------- | :------------------------------------ | :--------------------------------------------------------------------------------------------------- | :------------------------------------------ | :--------------------------------------- |
| **MCU**                  |             Mikropočítečovvá jednotka | Integrovaný čip (CPU + RAM + Flash na jednom substrátu), deterministický běh bez OS nebo RTOS        | např. ESP32, PIC16LF1xxx, RP2040            |                                          |
| **MPU**                  |             Mikroprocesor             | Samostatný procesor vyžadující externí RAM a úložiště, zpravidla běží plnohodnotný OS (Linux)        | např. Intel Core i5-14400                                            |                                          |
| **Embedded**             |             Vestavěné                 | Vestavěný mikropočítačová řídící jednotka, vestavěný mikroprocesor                                   | Embedded PLC, Embedded PC                        Bílá technika, bankomaty, regulace kotlů |
| **PLC**                  |        Programovatelný logický ovladač| Průmyslový automat pro cyklické deterministické řízení procesů, vysoká odolnost, modulární/kompaktní | SIEMENS SIMATIC S7-1500                                               |         |                       |
| **iPC**                  |                průmyslový počítač     | Vysoce odolný a spolehlivý počítačový systém zkonstruovaný pro nepřetržitý provoz v náročných provozních a průmyslových prostředích                                                                                                      |   IEC Stabil PRO ARBOR SB-142-1J64                                          |                                          |
| **Programovatelné relé** |           Malé PLC                            | Zjednodušené kompaktní PLC pro méně náročné úlohy, nahrazuje časovací relé a stykačové kombinace     |  SIEMENS 6ED1052-1MD08-0BA2 LOGO! 12/24RCE                                           |       
...
| :---------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :------------------------------ | :------------------------ | :----------------------------------- |
| **Doporučená platforma** *(MCU / PLC / iPC)*                                                          | **Programovatelné relé / kompaktní PLC** *(např. Siemens LOGO!, Eaton easyE4)*                                                                                                                                            | `MCU` Espressif ESP32-C6                           | `PLC`   Siemens SIMATIC S7-1200                  | `iPC`  Advantech MIC-770                              |
| **Pořizovací cena HW na 1 kus** *(nízká < 500 Kč / střední 5–30 tis. Kč / vysoká > 50 tis. Kč)*       | **Střední** *(cca 3 500 – 6 000 Kč)*                                                                                                                                                                                      | `Méně než 500Kč při sériové výrobě`                           | `střední`                     | `vysoká`                                |
| **Primární programovací jazyk** *(C/C++/MicroPython vs. IEC 61131-3 ST/LAD vs. Python/C#/C++ pod OS)* | **FBD / LAD** *(grafické funkční bloky nebo liniové schéma dle IEC 61131-3)*                                                                                                                                              | `C nebo C+`                           | `Python, Pascal nebo C`                     | `Python nebo C++`                                |
| **Klíčový technický argument pro volbu** *(např. spotřeba, determinismus, grafický výkon)*            | Montáž přímo na DIN lištu v rozváděči, integrovaný displej pro nastavení časovačů přímo na místě, robustní reléové výstupy pro motor a semafor, napájení 24 V DC / 230 V AC bez nutnosti vývoje vlastního plošného spoje. | `Nízká spotřeba energie pro bateriový provoz a integrovaná ZigBee/Wi-Fi konektivita`                           | `Vysoký determinismus a spolehlivost 24/7, podpora mnoha digitálních I/O`                     | `Vysoký výpočetní výkon pro zpracování 4K obrazu z AI neuronovou sítí`                                |
| **Hlavní riziko při volbě špatné platformy** *(proč by neuspěly ostatní dvě varianty)*                | **MCU:** Nutnost vývoje vlastní desky, nízká odolnost vůči venkovnímu rušení a obtížný servis údržbou.<br>**iPC:** Zbytečně extrémní cena (> 30 tis. Kč), dlouhý start po výpadku napájení a vysoká spotřeba.             | `PLC / iPC: Extrémní spotřeba (vybití baterie) a vysoká cena znemožňující masovou výrobu.`                           | `MCU: Chybějící průmyslová odolnost a složitý vývoj. iPC: Nestabilita OS a složitost pro podnikového elektrikáře`                     | `PLC / MCU: Nedostatečný výpočetní výkon a chybějící podpora pro komplexní AI modely a SQL.`                                |
